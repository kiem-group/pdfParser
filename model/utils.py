...
class StringUtils:
    ...
    def remove_apostrophed_words(s):
        s = re.sub("[a-zA-Z]+'", '', s)  # xx..'
        s = re.sub("'[a-zA-Z]", '', s)  # 'x
        return s

    @staticmethod
    def remove_iiis(s):
        s = re.sub(" [i]+ ", ' ', s)  # in the middle
        s = re.sub("^[i]+ ", ' ', s)  # at the beginning
        s = re.sub(" [i]+$", ' ', s)  # at the end
        return s

    @staticmethod
    def strip_single_letters_without_dot(s):
        s = re.sub(" [a-zA-Z] ", ' ', s)  # in the middle
        s = re.sub("^[a-zA-Z] ", ' ', s)  # at the beginning
        s = re.sub(" [a-zA-Z]$", ' ', s)  # at the end
        return s
